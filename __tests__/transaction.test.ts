...
test.only('Simple Transaction', async () => {
  ...
  const startInfo = await rds.query(`SELECT COUNT(*) AS cn FROM TestList${process.env.JEST_WORKER_ID}`);

  const results = await rds.transaction().then(async (transactionId) => {
    await rds.query(`INSERT INTO TestList${process.env.JEST_WORKER_ID} (data) VALUES (:data)`, { data: uuid() }, transactionId);
    await rds.query(`INSERT INTO TestList${process.env.JEST_WORKER_ID} (data) VALUES (:data)`, { data: uuid() }, transactionId);
    await rds.query(`INSERT INTO TestList${process.env.JEST_WORKER_ID} (data) VALUES (:data)`, { data: uuid() }, transactionId);
  ...
  const endInfo = await rds.query(`SELECT COUNT(*) AS cn FROM TestList${process.env.JEST_WORKER_ID}`);
...
test('Rollback Transaction', async () => {
  ...
  const startInfo = await rds.query(`SELECT COUNT(id) AS cn FROM TestList${process.env.JEST_WORKER_ID}`);
